...
def upload_and_notify(video_path, chat_id, bot_token, title=""):
    """Upload video to file.io and send download link to Telegram"""
    try:
        # Upload to file.io — fast server-to-server upload
        logger.info(f"Uploading video to file.io...")
        ...
        if not upload_response.ok:
            logger.error(f"file.io upload failed: {upload_response.status_code}")
            return False
        
        upload_data = upload_response.json()
        if not upload_data.get('success'):
            logger.error(f"file.io error: {upload_data}")
            return False
        
        file_url = upload_data.get('link')
        logger.info(f"Uploaded to file.io: {file_url}")

        # Send URL as text message to Telegram — takes 0.5 seconds
        telegram_url = f"https://api.telegram.org/bot{bot_token}/sendMessage"
        message = f"🎬 *{title}*\n\nYour video is ready\\!\n\n[Download here]({file_url})\n\n_Link expires in 24 hours\\. Download and post directly\\._"
        ...
        if msg_response.ok:
            logger.info("Download link sent to Telegram successfully")
            return True
        else:
            logger.error(f"Telegram message failed: {msg_response.status_code} {msg_response.text}")
            return False

    except Exception as err:
        logger.error(f"upload_and_notify error: {err}")
        return False
...
def process_render_job(data, job_id):
    ...
    try:
        ...
        # Upload to file.io and notify Telegram — FIRST before cleanup
        bot_token = os.environ.get('TELEGRAM_BOT_TOKEN')
        chat_id = os.environ.get('MOHAMMED_CHAT_ID')
        ...
        try:
            ...
            logger.error(f"Cleanup error (non-fatal): {cleanup_err}")
