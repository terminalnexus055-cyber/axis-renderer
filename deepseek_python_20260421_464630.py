...
def send_to_telegram(video_path, chat_id, bot_token, caption=""):
    """Send rendered video directly to Telegram chat"""
    try:
        logger.info(f"Sending video to Telegram chat {chat_id}")
        ...
        with open(video_path, 'rb') as video_file:
            response = requests.post(
                ...
                files={'video': ('output.mp4', video_file, 'video/mp4')},
                timeout=120
            )
        ...
        logger.error(f"send_to_telegram error: {err}")
        return False
...
def process_render_job(data, job_id):
    ...
    try:
        ...
        # Send video directly to Telegram — do this FIRST before cleanup
        ...
        if bot_token and chat_id:
            send_to_telegram(
                ...
                chat_id,
                bot_token,
                caption=f"🎬 *{title}*\n\n_Your video is ready. Download and post._"
            )
