...
def make_subtitle_bar(text, width, bar_height=120):
    ...
    return ImageClip(np.array(img), duration=None, transparent=True)
...
def render_video():
    try:
        ...
        logger.info(f"Starting render for video: {title}")
        ...
        for idx, scene in enumerate(scenes):
            ...
            if scene_type == 'text_only':
                ...
                if scene.get('subtitle'):
                    ...
                    subtitle_bar = make_subtitle_bar(subtitle_text, OUTPUT_WIDTH)
